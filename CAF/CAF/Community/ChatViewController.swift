        ...
        docReference?.collection("thread").addDocument(data: data, completion: { (error) in
          ...
          //  let newMesg = [self.currentUser:1]
            ...
            Database.database().reference().child("users").child(self.user2UID!).child("inbox").updateChildValues(diction) { (err, ref) in
                ...
                let postnoto1 = [Auth.auth().currentUser?.uid:self.user2UID!]

                // AGREGANDO POSIBLE NOTIFICACION

                Database.database().reference().child("MessageNoti").child(advengers.shared.currentChurchInfo.uidChurch).updateChildValues(postnoto1)
               // self.dismiss(animated: true, completion: nil)
            }
