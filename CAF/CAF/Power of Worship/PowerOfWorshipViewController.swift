...
class PowerOfWorshipViewController:  UIViewController, AVAudioPlayerDelegate{
    ...
    lazy var statusAudio: UILabel = {
        ...
        label2.text = NSLocalizedString("Loading audio file ...", comment: "")
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        navigationController?.navigationBar.backgroundColor = advengers.shared.colorBlue
               ...
               if advengers.shared.isPastor {
                   ...
                   navigationItem.rightBarButtonItem = UIBarButtonItem(title: NSLocalizedString("Select Channel", comment: ""), style: .plain, target: self, action: #selector(addPlaylist))
...
//        navigationItem.title = advengers.shared.currentChurch
        ...
        laber.text = NSLocalizedString("Your church is connected to this channel", comment: "")
    ...
    func loadActiveChannel (codigo: String) {
        ...
        let userPostRef = secondaryDb.reference().child("Media_Channels").child(codigo)
        //let userPostRef = Database.database().reference().child("Media_Channels")
        userPostRef.observeSingleEvent(of: .value, with: { (data) in
    ...
    func loadTrack (track: String, completionHandler: @escaping CompletionHandler) -> wSong {
        ...
        let userPostRef = secondaryDb.reference().child("Media_Songs").child(track)
...
//                          }
    ...
    func playNow (index: Int, segPosition: Int) {
        ...
        // Retrieve a Real Time Database client configured against a specific app.
        let secondarySt = Storage.storage(app: secondary)
        
        let httpsReference = secondarySt.reference(forURL: urlSongs[index].songURL)
        estadodelPlay.text = NSLocalizedString("Loading ..."
, comment: "")
        ...
        
    {
        ...
        do {
           ...
           // audioPlayer?.play(atTime: Double(segPosition))
            ...
            print("Playing ....")
            estadodelPlay.text = NSLocalizedString("Playing ....", comment: "")
        } catch {
            estadodelPlay.text = NSLocalizedString("No Playing - Error in Conection", comment: "")
            //print(url.absoluteString)
        }
