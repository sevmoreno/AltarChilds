...
class PhotoStreamViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout, ImageOnlyDelegate,textOnlyDelegate, textImageDelegate, AudibleDelegate  {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(nuevomensaje), name: NSNotification.Name(rawValue: "NuevoMensaje"), object: nil)
            ...
            if success {
                ...
                self.accountHelper.loadCurrentChurch(codigo: advengers.shared.currenUSer["churchID"] as! String,completionHandler: { (success) -> Void in
                    ...
                    if success {
                        ...
                        self.accountHelper.activeAplication()
    ...
    @objc func nuevomensaje () {
        
        accountHelper.nuevomensaje(controller: self)
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "reloadUsuariosChats"), object: nil)
    ...
    @objc func handleUpdateFeed() {
        ...
        photos.removeAll()
        collectionView.reloadData()
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: NSLocalizedString("+ Prayer", comment: ""), style: .plain, target: self, action: #selector(addprayer))
...
//                    print(self.view.frame.height)
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case advengers.postType.imageOnly.rawValue:
            ...
            _ = accountHelper.fetchUserPicURL(userID: photos[indexPath.row].userID ,completionHandler: { (nuevaFoto) -> Void in
                print(nuevaFoto)
                 cell.photoUsuario = nuevaFoto
                })
            
            
           // let accounthelper =  AccountHelpers ()
            //       let nuevaFoto = accounthelper.fetchUserPicURL(userID: (post?.userID)!)
